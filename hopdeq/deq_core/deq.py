...
class DEQ(torch.nn.Module):
    def __init__(
        ...
        # Make a distinction between forward and backward
        # Typically, backward convergence should be a lot better than forward
        def suffix_logger(suffix: str, logger):
            def new_logger(name: str, *args, **kwargs):
                return logger(name + suffix, *args, **kwargs)

            return new_logger

        forward_kwargs["logger"] = suffix_logger("_forward", logger)
        backward_kwargs["logger"] = suffix_logger("_backward", logger)

        # Store important attributes to enable external access
        ...
        # Make solver and backward methods from args
